...
class TestSplits:    
    def test_instancing(self, livesplit_vicecity):
        ...
        recreated_element = model.to_xml_tree(exclude_unset =True) #only include elements actually set by the XML
        recreated_element_string = ET.tostring(recreated_element, method='c14n2', strip_text=True).decode("utf-8")
